    ...
    for (const entry of entries) {
      ...
      const idx = (function () {
        if (config.options?.arrayFormat === "brackets") {
          return isNaN(Number(index)) ? index : "";
        } else {
          return index;
        }
      })();
...
export class HyperRequest {
  ...
  private async _request<T = any, D = any>(
    ...
    if (method === "GET" && requestConfig?.params) {
      url += "?" + JsonToUrlParams(requestConfig.params);
    }
    ...
    const res = await Promise.race([
      fetch(this.defaults.baseURL + url, init),
    ...
    if (res.status >= 400) {
      ...
      try {
        ...
        return Promise.reject(
          new RequestError(res.status, res.statusText || "Server Unknown Error")
        );
      }
